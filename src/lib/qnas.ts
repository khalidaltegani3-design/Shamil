...
export async function queryQNAS(zone: string, street: string, building: string): Promise<QNASResponse> {
  ...
  try {
    ...
    const possibleUrls = [
      ...
      // استخدام Google Geocoding كحل بديل
      `https://maps.googleapis.com/maps/api/geocode/json?address=${normZone}+${normStreet}+${normBuilding}+Qatar&key=${process.env.GOOGLE_MAPS_API_KEY}`
    ];
    ...
    // إضافة تشويش صغير للشارع والمبنى
    const streetOffset = (parseInt(normStreet) % 100) * 0.001;
    const buildingOffset = (parseInt(normBuilding) % 100) * 0.0005;
