...
export default function BottomNavbar() {
  ...
  return (
    <footer className={cn(
        "bottom-0 w-full p-3 z-10",
        isDiscoverPage ? "absolute" : "sticky"
    )}>
      <nav className={cn(
          "flex items-center justify-around h-16 rounded-xl",
          isDiscoverPage ? "bg-black/40 backdrop-blur-sm" : "bg-card shadow-lg"
      )}>
            ...
            <Link href={href} key={label} className="flex-1">
              <div
                className={cn(
                  ...
                  isActive
                    ? (isDiscoverPage ? "text-white" : "text-primary")
                    : (isDiscoverPage ? "text-gray-300 hover:text-white" : "text-muted-foreground hover:bg-accent/50 hover:text-primary")
                )}
              >
                <Icon className="h-6 w-6" />
                {!isDiscoverPage && <span>{label}</span>}
              </div>
            </Link>
          );
        })}
